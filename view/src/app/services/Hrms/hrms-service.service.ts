   ...
   ValidateEveryRequest() {
      ...
      public LeaveDate_AsyncValidate(Info: any): Observable<any[]> {
         if (this.Service.If_LoggedIn()) {
            ...
            return this.http.post(API_URL + 'LeaveDate_AsyncValidate', Info, {headers: this.headers }).pipe( map(response => response),  catchError(error => of(error)));
         }  else {
      ...
      public Expenses_Pay(Info: any): Observable<any[]> {
         if (this.Service.If_LoggedIn()) {
            this.headers.set('Authorization', atob(sessionStorage.getItem('SessionToken')));
            sessionStorage.setItem('SessionKey', btoa(Date()));
            return this.http.post(API_URL + 'Expenses_Pay', Info, {headers: this.headers }).pipe( map(response => response),  catchError(error => of(error)));
         }  else {
            return this.ValidateEveryRequest();
         }
      }
