   ...
   // Expenses Type Async Validate -----------------------------------------------
      exports.ExpensesType_AsyncValidate = function(req, res) {
         ...
         if(!ReceivingData.Expenses_Type || ReceivingData.Expenses_Type === '' ) {
            ...
            HrmsSettingsModel.ExpensesTypeSchema.findOne({'Expenses_Type': { $regex : new RegExp("^" + ReceivingData.Expenses_Type + "$", "i") }, 'If_Deleted': false }, {}, {}, function(err, result) {
               if(err) {
      ...
      exports.Expenses_Type_Create = function(req, res) {
         ...
         if(!ReceivingData.Expenses_Type || ReceivingData.Expenses_Type === '' ) {
            ...
            Create_ExpensesType.save(function(err, result) { // Expenses Type Save Query
               if(err) {
                  ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Creation Query Error', 'Hrms_Settings.controller.js');
                  ...
                  HrmsSettingsModel.ExpensesTypeSchema
                     ...
                     if(err_1) {
                        ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Find Query Error', 'Hrms_Settings.controller.js', err_1);
                        res.status(417).send({status: false, Message: "Some error occurred while Find The Expenses Types!."});
                     } else {
                        var ReturnData = CryptoJS.AES.encrypt(JSON.stringify(result_1), 'SecretKeyOut@123');
                           ReturnData = ReturnData.toString();
                        res.status(200).send({Status: true, Response: ReturnData });
      ...
      exports.Expenses_Type_List = function(req, res) {
         ...
         if (!ReceivingData.User_Id || ReceivingData.User_Id === ''  ) {
            ...
            HrmsSettingsModel.ExpensesTypeSchema
               ...
               if(err) {
                  ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Find Query Error', 'Hrms_Settings.controller.js', err);
                  res.status(417).send({status: false, Error:err, Message: "Some error occurred while Find The Expenses Types!."});
               } else {
                  var ReturnData = CryptoJS.AES.encrypt(JSON.stringify(result), 'SecretKeyOut@123');
                  ReturnData = ReturnData.toString();
                  res.status(200).send({Status: true, Response: ReturnData });
   ...
   // Expenses Type Simple List -----------------------------------------------
      exports.Expenses_Type_SimpleList = function(req, res) {
         ...
         if (!ReceivingData.User_Id || ReceivingData.User_Id === ''  ) {
            ...
            HrmsSettingsModel.ExpensesTypeSchema.find({'If_Deleted': false }, { Expenses_Type : 1 }, {sort: { updatedAt: -1 }}, function(err, result) { // Expenses Type FindOne Query
               if(err) {
                  ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Find Query Error', 'Hrms_Settings.controller.js', err);
                  res.status(417).send({status: false, Error:err, Message: "Some error occurred while Find The Expenses Types!."});
               } else {
      ...
      exports.Expenses_Type_Update = function(req, res) {
         ...
         if(!ReceivingData.Expenses_Type_Id || ReceivingData.Expenses_Type_Id === '' ) {
            ...
            HrmsSettingsModel.ExpensesTypeSchema.findOne({'_id': ReceivingData.Expenses_Type_Id}, {}, {}, function(err, result) { // Expenses Type FindOne Query
               if(err) {
                  ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type FindOne Query Error', 'Hrms_Settings.controller.js', err);
                  ...
                  if (result !== null) {
                     ...
                     result.save(function(err_1, result_1) { // Expenses Type Update Query
                        if(err_1) {
                           ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Update Query Error', 'Hrms_Settings.controller.js');
                           ...
                           HrmsSettingsModel.ExpensesTypeSchema
                              ...
                              if(err_2) {
                                 ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Find Query Error', 'Hrms_Settings.controller.js', err_2);
                                 res.status(417).send({status: false, Message: "Some error occurred while Find The Expenses Types!."});
                              } else {
                                 var ReturnData = CryptoJS.AES.encrypt(JSON.stringify(result_2), 'SecretKeyOut@123');
                                    ReturnData = ReturnData.toString();
               ...
               if(err) {
                  ...
                  if (result !== null) {
                     ...
                     result.save(function(err_1, result_1) { // Expenses Type Delete Query
                        if(err_1) {
                           ErrorManagement.ErrorHandling.ErrorLogCreation(req, 'CRM Settings Expenses Type Delete Query Error', 'Hrms_Settings.controller.js');
